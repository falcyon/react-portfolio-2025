...
function updateBackgroundShapePos(
  s: { state: ShapeState; scrollVal: number },
  i: number,
  scaledStates: { state: ShapeState; scrollVal: number }[],
  prevI: number,
  nextI: number
) {
  //changing position of random states to be closer to target when finally moving. 
  if (s.state.__random) {

    const midI = Math.floor((prevI + nextI) / 2)
    const prevStatewithText = scaledStates[prevI].state
    const nextStatewithText = scaledStates[nextI].state
    const midState = scaledStates[midI].state


    if (i < midI) {
      const t = (i - prevI) / (midI - prevI)
      s.state.x = prevStatewithText.x * (1 - t) + midState.x * t
      s.state.y = prevStatewithText.y * (1 - t) + midState.y * t
    }
    else if (i > midI) {
      const t = (i - midI) / (nextI - midI)
      s.state.x = midState.x * (1 - t) + nextStatewithText.x * t
      s.state.y = midState.y * (1 - t) + nextStatewithText.y * t
    }
  }
  return {
    state: s.state,
...
function scaleShape(
  ...
  // Precompute all indices of non-random states
  const validIndices = shape.states
    .map((s, idx) => (!s.state.__random ? idx : null))
    .filter((idx): idx is number => idx !== null);
  const scaledStates = shape.states.map((s, i) =>
    scaleState(s, i, shape, windowWidth, windowHeight, scaleFactor, isMobile)
  )

  const updatedstates = scaledStates.map((s, i) => {
    const prevI = [...validIndices].reverse().find(idx => idx < i) ?? 0;
    const nextI = validIndices.find(idx => idx > i) ?? 0;
    return updateBackgroundShapePos(s, i, scaledStates, prevI, nextI)
  })
